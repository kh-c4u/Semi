...
<body>
	...
	<div class="sub-right-content">
		...
		<div class="left pb5 mr5">
			<% if( m != null && m.getUserId().equals(b.getNwriterId())) { %>
			<a
				href="<%= request.getContextPath()%>/noticeupview.no?bno=<%=b.getNno()%>"
				class="btn3">수정</a>
			<% } %>
		</div>
			...
			<table class="tstyle2" style="word-break: break-all">
				...
				<tbody>
					...
					<tr>
						...
						<td class="pl20 r-noline"><span class="darkgray pr20"><%= b.getNwriterId() %></span>

						</td>
						<th class="tcenter vm">작성일</th>
						<td class="pl20 r-noline"><%= b.getNdate() %></td>
						<th class="tcenter vm">조회수</th>
						<td class="pl20"><%= b.getNcount() %></td>
					</tr>
				</tbody>
			</table>
			<table class="tstyle2" style="word-break: break-all">
		...
		<div class="comment">
			<div class="replyWriteArea">
				...
				<form action="<%= request.getContextPath()%>/ninsertComment.bo"
					method="post">
					<input type="hidden" name="writer" value="<%=m.getUserId()%>" /> <input
						type="hidden" name="bno" value="<%=b.getNno() %>" /> <input
   ...
   function updateConfirm(obj) {
      ...
      location.href="<%= request.getContextPath()%>/nupdateComment.bo?"
   ...
   function deleteReply(obj) {
      ...
      location.href="<%= request.getContextPath()%>/ncomentDelete.co"
      +"?cno="+cno+"&bno="+bno;
   }
   ...
   function reConfirm(obj) {
      ...
      location.href='<%= request.getContextPath()%>/ninsertComment.bo'
                 + '?writer=<%= m.getUserId() %>' 
