 ...
 */
@WebServlet("/erinsert.bo")
public class errorInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ErrorBoard cb = new ErrorBoard(title,content,writer,fileName);
		cb.setBtype(category);
		int result = 0;
		ErrorBoardService cs = new ErrorBoardService();
		result = cs.insertError(cb);
		
		if(result>0) {
			request.getRequestDispatcher("/errorList.bo").forward(request, response);
